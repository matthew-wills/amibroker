...
void EnterLongPosition(){
   if(ExitBar(_Symbol,timeFrame,magicNumber))return;
   double posSize = defaultPosSize;
   if (useMoneyManagement) posSize = MoneyManagementCalculator(_Symbol,posSizeStep);
   RefreshRates();
   if(tpLong != 0){ EnterPosition(_Symbol,OP_BUY,posSize,Ask,maxSlippage,Ask-stopLossPoints,Ask+tpLong*_Point,magicNumber,"LongBreakout");}
   else{EnterPosition(_Symbol,OP_BUY,posSize,Ask,maxSlippage,Ask-stopLossPoints,0,magicNumber,"LongBreakout");}
}
...
void EnterShortPosition(){   
   ...
   if (useMoneyManagement) posSize = MoneyManagementCalculator(_Symbol,posSizeStep);
   RefreshRates();
   if(tpShort != 0){ EnterPosition(_Symbol,OP_SELL,posSize,Bid,maxSlippage,Bid+stopLossPoints,Bid-tpShort*_Point, magicNumber,"ShortBreakout");}
   else{ EnterPosition(_Symbol,OP_SELL,posSize,Bid,maxSlippage,Bid+stopLossPoints,0, magicNumber,"ShortBreakout");}
}
