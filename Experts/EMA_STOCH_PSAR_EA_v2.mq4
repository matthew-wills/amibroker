...
extern string     ema_Entry_Settings     = "----- ema_Entry Settings -----";
extern int ema_Entry_Fast_Period = 5;
extern int ema_Entry_Slow_Period = 20;

extern string     ema_Exit_Settings     = "----- ema_Exit Settings -----";
extern int ema_Exit_Fast_Period = 2;
extern int ema_Exit_Slow_Period = 10;
...
void Load_Indicator_Variables(){
   emaEntryFast_1 = iMA(_Symbol,timeFrame,ema_Entry_Fast_Period,0,MODE_EMA,PRICE_CLOSE,1);
   emaEntrySlow_1 = iMA(_Symbol,timeFrame,ema_Entry_Slow_Period,0,MODE_EMA,PRICE_CLOSE,1);
   emaExitFast_1 = iMA(_Symbol,timeFrame,ema_Exit_Fast_Period,0,MODE_EMA,PRICE_CLOSE,1);
   emaExitSlow_1 = iMA(_Symbol,timeFrame,ema_Exit_Slow_Period,0,MODE_EMA,PRICE_CLOSE,1);
...
int Signal(){
   if( emaEntryFast_1 > emaEntrySlow_1 && close_1 > psar_1 && stochMain_1 > stochSignal_1){
      return(1);
   }
   if( emaEntryFast_1 < emaEntrySlow_1 && close_1 < psar_1 && stochMain_1 < stochSignal_1 ){
      return(-1);
...
void EnterShortPosition(){
   EnterPosition(_Symbol,OP_SELL,fixedLotSize,Bid,maxSlippage,0,0,magicNumber,"");
}

void TradeManagerLong(){
   if(emaExitFast_1 < emaExitSlow_1){
...
void GenerateComments(){   
   ...
   com+="\n-MA_Fast_Period = " + IntegerToString(ema_Entry_Fast_Period);
   com+="\n-MA_SLow_Period = " + IntegerToString(ema_Entry_Slow_Period);
   com+="\n-MA_Fast_Period = " + IntegerToString(ema_Exit_Fast_Period);
   com+="\n-MA_SLow_Period = " + IntegerToString(ema_Exit_Slow_Period);
   ...
   com+="\n-ema_Exit_Fast[1] = " + DoubleToString(emaExitFast_1,_Digits);
   com+="\n-ema_Exit_Slow[1] = " + DoubleToString(emaExitSlow_1,_Digits);
