...
string systemName = "Incrementum_RSI_Breakout";
...
extern int rsiPeriod = 6;// RSI length
extern double rsiEntryLong = 70;// RSI entry long
extern double rsiEntryShort = 30;// RSI entry short
...
void Load_Indicator_Variables(){
   ...
   rsi1 = iRSI(_Symbol,timeFrame,rsiPeriod,PRICE_CLOSE,1);
   rsi2 = iRSI(_Symbol,timeFrame,rsiPeriod,PRICE_CLOSE,2);
}
...
void OpenNewTrades(){
   ...
   if(rsi1 > rsiEntryLong && rsi2 < rsiEntryLong){
   ...
   if(rsi1 < rsiEntryShort && rsi2 >rsiEntryShort){
...
void GenerateComments(){   
   ...
   com+="\n-RSI("+(string)rsiPeriod+") = " + DoubleToString(rsi1,2);
   if(numOpenExpert!=0){
      com+="\n\n====================";
      com+="\n Open Position Info ";
      com+="\n====================\n"; 
