...
int maxSlippage = 2;// Max Acceptable Slippage 
//+------------------------------------------------------------------+
...
int Signal(){
   if(   emaEntryFast_1 > emaEntrySlow_1 && 
         close_1 > psar_1 && 
...
void EnterLongPosition(){
   EnterPosition(_Symbol,OP_BUY,fixedLotSize,Ask,maxSlippage,Ask - stopLoss*_Point, Ask + takeProfit*_Point,magicNumber,"");
}

void EnterShortPosition(){
   EnterPosition(_Symbol,OP_SELL,fixedLotSize,Bid,maxSlippage,Bid + stopLoss*_Point, Bid - takeProfit*_Point, magicNumber,"");
}
