...
extern string expertName = "Sitting_Duck";          // Name of Trading System
extern datetime comissionDate = D'2016.01.01';      // YYYYMMDD  
...
input bool ATR_Filter_Toggle = false; //ATR Filter ON/OFF
input int ATR_Filter_P1 = 1;          //ATR Filter P1
input int ATR_Filter_P2 = 20;         //ATR Filter P2
...
extern double takeProfit = 1.5;          //takeProfit Percent Value
...
{  
   ...
   string expertSettings = StringConcatenate
      (      
          ...
          newLine, "volatilityFilter = ", ATR_Filter_Toggle," / ",ATR_Filter_P1," / ",ATR_Filter_P2,
...
//////////////////////////////--- Check for a valid entry signal ---///////////////////////////////////
      ...
      // Is the ATR Filter ON
      if(ATR_Filter_Toggle == true)
      {
         // If Filter is on and Filter is False disarm the symbol
         if( ATR_Filter(symbol,timeFrame,ATR_Filter_P1,ATR_Filter_P2) == false )
         {
            ARMED[i] = 0;
            SCANNED[i] = 99;         
            continue;
      ...
      // Is RSI1 <= RSI Entry Limit
      double rsi1 = -1;
      rsi1 = customRSI( symbol, timeFrame, rsiPeriod, 1 );
      if( rsi1 == -1 )
      {
         if( errorPrint )
         {
            Print( symbol, " RSI Variable Error ... " );
         }
      ...
      double entryPrice = 0;
      entryPrice = iLow( symbol, timeFrame, 1 ) * ( 100 - entryLimit ) * 0.01;
      
      if( entryPrice == 0 )
      {
         if( errorPrint )
         {
            Print( symbol, " ASK or LOW Variable Error ... " );
         }
