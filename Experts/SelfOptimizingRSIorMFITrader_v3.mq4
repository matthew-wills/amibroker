...
input int inIndyPeriods=4; // Averaging period for index and ATR calculations
...
void OnTick()
  {
   ...
   if(BarsSinceEntry(Symbol(),NULL,magic)>=7)// Check for nBarExit
   {
      RefreshRates();
      ExitLong(Symbol(),Bid,10,magic,clrRed);
   }  
      ...
      if((currIndyValue<os && prevIndyValue>os) || inAggressive)//buy
        {
         if(CheckMoneyForTrade(NULL,lotsize,OP_BUY) && VolumeGood)
           {
            ticket = OrderSend(NULL,OP_BUY,lotsize,Ask,10,Bid*0.8,Bid*1.1,"Buy Order",magic,0,clrGreen);
         ...
         if((currIndyValue<os && prevIndyValue>os) || inAggressive)//buy
           {
            if(CheckMoneyForTrade(NULL,lotsize,OP_BUY) && VolumeGood)
              {
               ticket = OrderSend(NULL,OP_BUY,lotsize,Ask,10,Bid*0.5,Bid*1.5,"Buy Order",magic,0,clrGreen);
               Print("Best ",EnumToString(index)," value for buy at ",buy," current ",EnumToString(index)," value: ",currIndyValue,
                     " Old ",EnumToString(index)," ",prevIndyValue," Backtest Profits :: (BUY ",buyProfits,") : SELL ",sellProfits);
              }
           }
            ...
            for(int remainingBars=i-1; remainingBars>0; remainingBars--)
              {
               ...
               if(orderOn && optimizer[remainingBars].close<orderOpenPrice*0.01)//-(stoploss*Point))
                 {
                  orderOn=false; // Turn the order off since stoploss was hit
                  sOverSold[r].loss++; // add to loss counter
                  i=remainingBars; // skip the bars used while the trade was on
                  sOverSold[r].pipValue = sOverSold[r].pipValue + (optimizer[remainingBars].close-orderOpenPrice)/Point; // add up totalProfit
                  break;
                 }//( orderOn && optimizer[remainingBars].close > orderOpenPrice + (stoploss*Point))
               
               // Check if it hits takeProfit for a buy
               if(orderOn && optimizer[remainingBars].close>orderOpenPrice*1.99)//(takeprofit*Point))
                 {
                  orderOn=false;// Turn the order off since stoploss was hit
               ...
               // Check if it hits nBarExit for a buy
               if(orderOn && (i - remainingBars) > 7)
                 {
                  orderOn=false;// Turn the order off since stoploss was hit                  
                  profitCheck = (optimizer[remainingBars].close-orderOpenPrice)/Point;
                  if(profitCheck > 0)
                     {
                        sOverSold[r].profit++; // add to profit counter
                        i=remainingBars; // skip the bars used while the trade was on
                        sOverSold[r].pipValue = sOverSold[r].pipValue + profitCheck; // add up totalProfit
                        break;  
                     }
                     else
                     {
