...
string expertName = "GBO_Short";

extern string     SYSTEM_SETTIGNS      = "----- System Settings -----";
extern int magicNumber = 321;                 //Magic Number
...
extern string     ORDER_MANAGEMENT_SETTINGS    = "----- Order Management Settings -----";
...
//---
   ...
   if( CountSystemOrdersShort(magicNumber) > 0 && operationMode != "TRADE_MANAGER")
   {
...
void CHECKINPUTDATA()
{
   ...
   {
      AlertUser("takeProfitPrice cannot be greater than BreakoutPrice... Expert will terminate");
      TerminateEA();
   }
   
   if(stopLoss_PriceOnOff==true && stopLoss_Price < condition1_Limit)
   {
      AlertUser("stopLossPrice cannot be less than BreakoutPrice... Expert will terminate");
      TerminateEA();
   }
   
   if(stopLoss_CandleOnOff==true && stopLoss_Candle_Price < condition1_Limit)
   {
      AlertUser("stopLoss_candle_price cannot be less than BreakoutPrice... Expert will terminate");
      TerminateEA();
   }
...
void ENTRY_MANAGER()
{
   ...
   if( iTime(Symbol(),primaryTimeFrame,0) != primaryTimeFrame_time ) 
   {
      ...
      {
         OpenOrderShort("Breakout_Check_Positive");
         return;
      }
   ...
   Comment(
      newLine, "GBO SYSTEM - SHORT",Animation(cycleCounter),
...
void CONDITION2_CHECK()
{
   ...
   if( iTime(Symbol(),secondaryTimeFrame,0) != secondaryTimeFrame_time ) 
   {
      ...
      if( iHigh(Symbol(),secondaryTimeFrame,1) < c2_1)
      {
         OpenOrderShort("Check Result Positive 2.1");
         return;
      }    
      
      if(iClose(Symbol(),secondaryTimeFrame,1) < c2_2)
      {
         OpenOrderShort("Check Result Positive 2.2");
         return;
      }
   ...
   Comment(
      ...
      newLine,
      newLine, "CONDITION 2.1 MAXIMUM PRICE = ", DoubleToString(condition2_UpperLimit,5),
      newLine, "CONDITION 2.2 MAXIMUM PRICE = ", DoubleToString(condition2_2_Limit,5)     
   );
}
...
void TRADE_MANAGER()
{
   ...
   if( CountSystemOrdersShort(magicNumber) == 0 && tradeOpenedFlag > 0)
   {
   ...
   if(tralingStopCandle_timeFrame_AUX != trailingStopCandle_timeFrame || trailingStopNumCandles_AUX != trailingStopNumCandles || trailingStopCandlePrice_AUX != trailingStopCandlePrice)
   {
      ...
      TS_Candle_Value = trailingStopCandleOnOff*(HighestHighValue(Symbol(),trailingStopCandle_timeFrame,trailingStopNumCandles,1) + trailingStopCandlePrice);  
   }
   ...
   if( orderOpenPrice == 0)
   {
      orderOpenPrice = OpenPriceShort(Symbol(),magicNumber);
      TS_Price_Value = trailingStop_PriceOnOff*(orderOpenPrice + trailingStop_Price);
      TS_Candle_Value = trailingStopCandleOnOff*(HighestHighValue(Symbol(),trailingStopCandle_timeFrame,trailingStopNumCandles,1) + trailingStopCandlePrice);      
   }
   ...
   {
      if( Ask > stopLoss_Price )
      {
         ExitOrderShort("stopLoss_Price");
      }
   }
   //Check trailingStop_Price
   if(trailingStop_PriceOnOff == true)
   {
      if( Ask > TS_Price_Value)
      {
   ...
   //Trailing Stop Price
   if( Ask + trailingStop_Price < TS_Price_Value - trailingStop_Price_MinStep )
   {
      TS_Price_Value = Ask + trailingStop_Price;
   }
   
   
   //Trailing Stop Candle
   
   if( HighestHighValue(Symbol(),trailingStopCandle_timeFrame,trailingStopNumCandles+1,1) + trailingStopCandlePrice < TS_Candle_Value )
   {
      TS_Candle_Value = HighestHighValue(Symbol(),trailingStopCandle_timeFrame,trailingStopNumCandles+1,1) + trailingStopCandlePrice;
   }
