...
void RefreshVariables(){
   ...
   candleSize = MathAbs((close1 - open1) / _Point);
...
void GenerateComments(){
   ...
   com+="\n-candleTime = " + (string)iTime(_Symbol,timeFrame,1);
   ...
   com+="\n=========================";
   com+=  "\n checkDistance = " + (string)CheckDistance();
   com+=  "\n checkTrendDirection = " + (string)CheckTrendDirection();
   com+=  "\n checkSignal = " + (string)CheckSignal();
   com+=  "\n checkStrength = " + (string)CheckStrength(_Symbol);
   com+=  "\n RelativeHIGH = " + (string)RelativeHigh();
   com+=  "\n RelativeLOW = " + (string)RelativeLow(); 
   Comment(com);
}
int CheckSignal(){
...
double RelativeHigh(){
   for( int i = 2; i <= 99 ; i++ ){
      if( iClose(_Symbol,timeFrame,i) > iOpen(_Symbol,timeFrame,i) ){
         if( close1 > iClose(_Symbol,timeFrame,i) ){
            return((close1 - iClose(_Symbol,timeFrame,i)));
         }
         else{
            return(0);
         }
...
double RelativeLow(){
   for( int i = 2; i <= 99 ; i++ ){
      if( iClose(_Symbol,timeFrame,i) < iOpen(_Symbol,timeFrame,i) ){
         if( close1 < iClose(_Symbol,timeFrame,i) ){
            return((iClose(_Symbol,timeFrame,i) - close1));
         }
         else{
            return(0);
         }
...
int CheckTrendDirection(){
   if(close1 > prevDayClose)return(1);
   if(close1 < prevDayClose)return(-1);
   return(0);
}
...
int CheckDistance(){
   double distance = 0;
   if(prevDayClose != 0){
      distance = MathAbs(close1 - prevDayClose) / prevDayClose * 100;
   }
...
bool TimeCheck(int _start, int _finish){
   ...
   int currentTime = TimeConvert(iTime(_Symbol,timeFrame,1));
...
double StopLoss_Long(){
   for( int i = 2; i <= 99 ; i++ ){
      if( iClose(_Symbol,timeFrame,i) < iOpen(_Symbol,timeFrame,i) ){
         return(NormalizeDouble(MathMin(iLow(_Symbol,timeFrame,i),iLow(_Symbol,timeFrame,i-1)) - StopLoss_Points * _Point,_Digits));
      }
   }
   return(0);   
}
...
double StopLoss_Short(){
   for( int i = 2; i <= 99 ; i++ ){
      if( iClose(_Symbol,timeFrame,i) > iOpen(_Symbol,timeFrame,i) ){
         return(NormalizeDouble(MathMax(iHigh(_Symbol,timeFrame,i),iHigh(_Symbol,timeFrame,i-1)) + StopLoss_Points * _Point,_Digits));
      }
   }
   return(0);   
}
...
bool EnterLongPosition( string _symbol, double _lots, double _price, int _slippage, double _stopLoss, double _takeProfit, int _magicNumber, string _label )
{
   ...
   if( !OrderSend( _symbol, ORDER_TYPE_BUY, lots, _price, _slippage, stoploss, takeprofit, _label, _magicNumber, 0, Green ) )
   {
...
bool EnterShortPosition( string _symbol, double _lots, double _price, int _slippage, double _stopLoss, double _takeProfit, int _magicNumber, string _label )
{
   ...
   if( !OrderSend( _symbol, ORDER_TYPE_SELL, lots, _price, _slippage, stoploss, takeprofit, _label, _magicNumber, 0, Green ) )
   {
     ErrorReport( GetLastError() );
     return(false);
   }
